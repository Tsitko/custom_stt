...
## Как работает STT
- Отдельное окружение `.venv-stt-gpu` для GPU STT (чтобы не конфликтовало с TTS/torch). Сервис `/stt` использует его и держит модель в памяти через `stt_worker.py`.
- Воркер держит модель в памяти 5 минут после последнего запроса (TTL в `service.py`), что убирает холодный старт для частых запросов.
- CLI `stt_app.py` по-прежнему читает `config.yml`, инициализирует `stt/gigaam_transcriber.py` (GigaAM-v3, устройство из `stt_device`) и выдаёт сырой текст.
