#!/usr/bin/env python3
"""Persistent Qwen ASR worker process for GPU inference."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Persistent Qwen ASR worker.")
...
def main() -> int:
    ...
    model_name = args.model or config.qwen_asr_model
    device = args.device or config.qwen_asr_device
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            logging.info("Qwen ASR worker received context length: %s", context_len)
            if not audio_path.exists():
                raise FileNotFoundError(audio_path)
            rms_db = compute_rms_dbfs(audio_path)
            if rms_db is not None:
                logging.info("Qwen ASR RMS level: %.2f dBFS (threshold %.2f)", rms_db, config.stt_silence_db)
            ...
            logging.info("Qwen ASR worker timings: infer=%.3fs llm=%.3fs", infer_elapsed, llm_elapsed)
