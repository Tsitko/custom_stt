...
- При рестарте сервис принудительно выгружает модели и завершает процесс даже во время генерации, чтобы не зависать на остановке.
...
- При `use_llm_tts: false` текст идёт напрямую в движок.
